...
export async function getHistoryCandles(params: {
  chainIndex: string;
  ...
  if (params.limit) {
    ...
    if (!Number.isFinite(limit) || limit <= 0 || limit > 300) {
      throw new Error("limit must be a numeric string between 1 and 300");
    }
    query.set("limit", params.limit);
  }

  const requestPath = `${OKX_HISTORY_CANDLES_PATH}?${query.toString()}`;
